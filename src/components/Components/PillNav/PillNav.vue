...
const isActiveItem = (itemHref?: string) => {
  ...
  if (props.activeHref) return props.activeHref === itemHref;
...
watch(() => [props.items, props.ease, props.initialLoadAnimation], layout, { deep: true });

const handleEnter = (i: number) => {
  const tl = tlRefs.value[i];
  if (!tl) return;
  activeTweenRefs.value[i]?.kill();
  activeTweenRefs.value[i] = tl.tweenTo(tl.duration(), { duration: 0.3, ease: props.ease, overwrite: 'auto' });
};
...
const handleLeave = (i: number) => {
  ...
  if (!tl) return;
  activeTweenRefs.value[i]?.kill();
  activeTweenRefs.value[i] = tl.tweenTo(0, { duration: 0.2, ease: props.ease, overwrite: 'auto' });
};
...
const handleLogoEnter = () => {
  ...
  logoTweenRef.value = gsap.to(img, { rotate: 360, duration: 0.2, ease: props.ease, overwrite: 'auto' });
};
...
const toggleMobileMenu = () => {
  ...
  if (menu) {
    if (newState) {
      ...
      gsap.fromTo(menu, { opacity: 0, y: 10, scaleY: 1 }, { opacity: 1, y: 0, scaleY: 1, duration: 0.3, ease: props.ease, transformOrigin: 'top center' });
    } else {
      gsap.to(menu, { opacity: 0, y: 10, scaleY: 1, duration: 0.2, ease: props.ease, transformOrigin: 'top center', onComplete: () => { gsap.set(menu, { visibility: 'hidden' }); } });
    }
  }
  props.onMobileMenuClick?.();
};

const isExternalLink = (href: string) => href.startsWith('http') || href.startsWith('//') || href.startsWith('#');
...
<template>
  <div class="relative w-auto">
    <nav
      :class="['flex items-center justify-end box-border', className]"
      aria-label="Primary"
      :style="cssVars"
    >
      <div
        ref="navItemsRef"
        class="hidden relative md:flex items-center rounded-full transition-all duration-300 ease-out"
        :style="{
          height: 'var(--nav-h)',
      ...
      >
        <ul role="menubar" class="flex items-stretch m-0 p-[3px] h-full list-none" :style="{ gap: 'var(--pill-gap)' }">
          <li 
            ...
            class="flex h-full transition-all duration-500 ease-[cubic-bezier(0.25,0.8,0.25,1)]"
            :style="{
              margin: isActiveItem(item.href) ? '0 20px' : '0 0'
            }"
            role="none"
          >
            <component
              ...
              class="inline-flex box-border relative justify-center items-center px-0 rounded-full h-full overflow-hidden font-semibold text-[16px] no-underline uppercase leading-[0] tracking-[0.2px] whitespace-nowrap cursor-pointer transition-all duration-300"
              :style="{
      ...
      >
        <span class="rounded w-4 h-0.5 origin-center transition-all duration-[10ms] ease-[cubic-bezier(0.25,0.1,0.25,1)] hamburger-line" :style="{ background: 'var(--pill-bg, #fff)' }" />
        <span class="rounded w-4 h-0.5 origin-center transition-all duration-[10ms] ease-[cubic-bezier(0.25,0.1,0.25,1)] hamburger-line" :style="{ background: 'var(--pill-bg, #fff)' }" />
      </button>
    </nav>
    ...
    <div
      ref="mobileMenuRef"
      class="md:hidden fixed top-[5rem] left-4 right-4 z-[998] shadow-[0_8px_32px_rgba(0,0,0,0.12)] rounded-[27px] origin-top"
      :style="{ ...cssVars, background: 'var(--base, #f0f0f0)', backdropFilter: 'blur(10px) saturate(180%)', WebkitBackdropFilter: 'blur(10px) saturate(180%)' }"
    >
